...
@section('title', 'Outstanding PO Ingestion - CRM Reconciliation')
    ...
    <div class="card">
        ...
        @if ($errors->any())
            <div class="alert alert-danger">
                ...
                @if (session('po_parser_debug_sample'))
                    <div style="margin-top: 1rem; background-color: rgba(0, 0, 0, 0.2); border: 1px solid rgba(255,255,255,0.08); border-radius: 0.5rem; padding: 1rem; text-align: left;">
                        <strong style="color: #fcd34d; font-size: 0.85rem; display: block; margin-bottom: 0.5rem;">Detected Rows Sample (First 30 Rows):</strong>
                        <pre style="font-family: monospace; font-size: 0.75rem; color: #a7f3d0; max-height: 250px; overflow-y: auto; white-space: pre-wrap; margin: 0; line-height: 1.4;">{{ session('po_parser_debug_sample') }}</pre>
                    </div>
                @endif
            </div>
        @endif

        <div style="background-color: rgba(139, 92, 246, 0.05); border: 1px solid rgba(139, 92, 246, 0.15); border-radius: 0.5rem; padding: 0.85rem; margin-bottom: 1.25rem; font-size: 0.8rem; color: #ddd6fe; line-height: 1.5;">
            💡 <strong>Ingestion Rules:</strong><br>
            - Multiple active PO items per product/customer are permitted.<br>
            - *Sales order lines containing local allocated shipment history are locked and skipped to preserve data integrity.
        </div>
